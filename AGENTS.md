...
- **LLM Provider Chain**: [`data/ai_scraper.py`](data/ai_scraper.py:444) `call_llm()` fallback order: Kilo Gateway (`kilo-auto/free`, primary) → OpenCode Zen (`big-pickle`) → HuggingFace → Groq (`llama-3.3-70b-versatile`) → Gemini. Each provider has its own rate limiter and dead-provider caching (skip for 600s after failure). Uses `curl_cffi` with TLS 1.2 adapter to bypass Windows OpenSSL SSL bugs. Config keys under `scrapegraphai`: `kilo_api_key`, `opencode_api_key`, `groq_api_key`, `openrouter_api_key`, `sambanova_api_key`, `saas_api_key`.
