...
#  DATA LAYER ΓÇö persist to JSON
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
def enter_trade(alert: dict) -> dict:
    ...
    """
    # Market-hours gate ΓÇö applies to paper AND future live broker trades.
...
def check_and_close_trades() -> list[dict]:
    ...
    # Outside market hours and outside the EOD flatten window ΓÇö do nothing.
...
def _build_option_price_map(open_ops: list[dict]) -> dict[str, dict[str, float]]:
    ...
    Returns dict keyed by (strike, expiry, type) ΓåÆ current_ltp."""
    try:
...
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
#  EOD ANALYSIS ΓÇö generates P&L summary + strategy corrections
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
def _analyze_trades(trades: list[dict]) -> dict:
    ...
    if winners:
        ...
        if tgt_hits:
            right.append(f"{len(tgt_hits)} trade(s) hit full target ΓÇö R:R framework working")
    ...
    if losers:
        ...
        if low_conf_losers:
            wrong.append(f"{len(low_conf_losers)} loser(s) were LOW/MEDIUM confidence ΓÇö filter stricter")
        if len(sl_hits) > len(tgt_hits):
            wrong.append(f"SL hits ({len(sl_hits)}) > Target hits ({len(tgt_hits)}) ΓÇö SL too tight or entries poorly timed")
        ...
        if len(long_losers) > 2:
            wrong.append(f"Multiple long losses ({len(long_losers)}) ΓÇö bull signal may be false")
        if len(short_losers) > 2:
            wrong.append(f"Multiple short losses ({len(short_losers)}) ΓÇö bearish signal may be false")
    ...
    if total > 0:
        ...
        if win_rate >= 60:
            patterns.append(f"Win rate {win_rate:.0f}% ΓÇö system performing well")
        elif win_rate <= 30:
            patterns.append(f"Win rate {win_rate:.0f}% ΓÇö system underperforming, widen SL or reduce position count")
    ...
    if not wrong:
        wrong.append("No losing trades today ΓÇö clean session!")
...
def _generate_corrections(today_trades: list[dict], history: list[dict]) -> list[str]:
    ...
    if eod_profitable:
        corrections.append(f"TRAIL STOP: {len(eod_profitable)} trade(s) exited at EOD with profit ΓÇö implement trailing SL to lock gains.")
